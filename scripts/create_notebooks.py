#!/usr/bin/env python3
"""Script to create all foundation notebooks for the coursework structure"""
...
def create_module_1_3():
    """Module 1.3: Feed-Forward Networks & Normalization"""
    cells = [
        create_notebook_cell("markdown", """# Module 1.3: Feed-Forward Networks & Normalization
...
- Memory profiling"""),
        create_notebook_cell("markdown", "## Setup"),
        create_notebook_cell("code", "!pip install torch numpy matplotlib seaborn -q"),
        create_notebook_cell("code", """import torch
...
plt.style.use('seaborn-v0_8-darkgrid')"""),
        create_notebook_cell("markdown", """## Lesson 1: Activation Functions (15 mins)

Compare ReLU, GELU, and SwiGLU activation functions."""),
        create_notebook_cell("code", """# Define activation functions
def relu(x):
    return F.relu(x)
...
plt.tight_layout()
plt.show()"""),
        create_notebook_cell("markdown", """## Lesson 2: Feed-Forward Networks (15 mins)

Implement FFN with different activation functions."""),
        create_notebook_cell("code", """class FFN_ReLU(nn.Module):
...
print(f"SwiGLU FFN output: {out_swiglu.shape}")"""),
        create_notebook_cell("markdown", """## Lesson 3: Normalization Layers (15 mins)

Compare LayerNorm and RMSNorm."""),
        create_notebook_cell("code", """class RMSNorm(nn.Module):
...
print(f"RMSNorm mean: {out_rms.mean().item():.6f}, std: {out_rms.std().item():.6f}")"""),
        create_notebook_cell("code", """# Memory profiling comparison
import torch
...
for name, model in models.items():
    ...
    if mem > 0:
        ...
        create_notebook_cell("markdown", """## Key Takeaways
...
# Create the notebook
notebook = create_module_1_3()
os.makedirs("notebooks/foundations", exist_ok=True)
with open("notebooks/foundations/03_feedforward_normalization.ipynb", "w") as f:
    json.dump(notebook, f, indent=1)
print("Created Module 1.3 notebook")
